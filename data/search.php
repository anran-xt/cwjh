...
if($rq!=0&&$zt!=0){
    $sql = "select Ocode,Origin,Ostatus,OsendTime,OrderTime,Oname,address,telphone,notes,commodity,commCode,spci,commPri,commNum  from orderform where (UNIX_TIMESTAMP('$OrderTime') - UNIX_TIMESTAMP(OrderTime))/(60*60*24)<'$rq' and Ostatus='$zt';";
}else if($rq>0&&$rq<32&&$zt==0){
    $sql = "select Ocode,Origin,Ostatus,OsendTime,OrderTime,Oname,address,telphone,notes,commodity,commCode,spci,commPri,commNum  from orderform where (UNIX_TIMESTAMP('$OrderTime') - UNIX_TIMESTAMP(OrderTime))/(60*60*24)<'$rq';";
}else if($rq==32&&$zt==0){
    $sql = "select Ocode,Origin,Ostatus,OsendTime,OrderTime,Oname,address,telphone,notes,commodity,commCode,spci,commPri,commNum  from orderform where (UNIX_TIMESTAMP('$OrderTime') - UNIX_TIMESTAMP(OrderTime))/(60*60*24)>30;";
}else if($rq==32&&$zt!=0){
    $sql = "select Ocode,Origin,Ostatus,OsendTime,OrderTime,Oname,address,telphone,notes,commodity,commCode,spci,commPri,commNum  from orderform where (UNIX_TIMESTAMP('$OrderTime') - UNIX_TIMESTAMP(OrderTime))/(60*60*24)>30 and Ostatus='$zt';";
}
else if($zt!=0&&$rq==0){
    $sql = "select Ocode,Origin,Ostatus,OsendTime,OrderTime,Oname,address,telphone,notes,commodity,commCode,spci,commPri,commNum  from orderform where Ostatus='$zt';";
}else if($rq==0&&$zt==0){
